...
export async function sendWhatsAppMessage(toPhone: string, messageText: string): Promise<boolean> {
  try {
    ...
    console.log(`[WhatsApp Service] Envando mensaje de WhatsApp de ${from} a ${to}...`);
    ...
    console.error('❌ Error al enviar mensaje de WhatsApp por Twilio:', error.response?.data || error.message);
    return false;
  }
}
